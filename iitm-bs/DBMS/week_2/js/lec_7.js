...
function renderContext(){
  ...
  card.innerHTML = `
    <div class="sf-context-badge">CONCEPT ${state.qIdx+1} / ${QUESTIONS.length}${alreadyAnswered ? ' · <span style=\"color:var(--sf-text-muted)\">REVIEWED</span>' : ''}</div>
    <div class="sf-label">Context</div>
    <div class="sf-content">${processContent(q.context)}</div>
    ${q.tables ? parseTableText(q.tables) : ''}
    <p class="sf-next-hint" style="margin-top:auto;padding-top:12px;">${alreadyAnswered
      ? 'This question has already been answered. Press Enter or click Review to view your result.'
    ...
    <div class="sf-actions" style="margin-top:14px;">
      ...
      <button class="sf-action" id="btn-ctx-next" tabindex="0">${alreadyAnswered ? 'REVIEW ▶' : 'CONTINUE ▶'}</button>
    </div>`;
  renderMath();
...
// Read-only replay of an already-answered question
function renderReview(idx){
  ...
  const prev = state.answered[idx];           // {correct, chosen}
  const card = document.getElementById('sf-card');
  const keys = Object.keys(q.options);

  const optHtml = keys.map((k,i) => {
    let cls = 'sf-opt-btn';
    if(k === q.answer)                          cls += ' correct-reveal';
    else if(prev && k === prev.chosen)          cls += ' wrong-reveal';
  ...
  card.innerHTML = `
    <div class="sf-label" style="color:var(--sf-text-muted)">Review — Question ${idx+1} <span style="font-size:10px;letter-spacing:.1em;">(READ ONLY)</span></div>
    <div class="sf-content">${processContent(q.question)}</div>
    <ul class="sf-options">${optHtml}</ul>
    <div class="sf-feedback ${feedbackCls}">${feedbackTxt}</div>
    <div class="sf-explanation">${processContent(q.explanation)}</div>
    ${q.application ? `<div class="sf-application"><strong>▸ APPLICATION</strong> — ${processContent(q.application)}</div>` : ''}
...
function renderQuestion(){
  if(guardQuestion()) return;   // redirect to read-only review if already answered
  const q = QUESTIONS[state.qIdx];
  const card = document.getElementById('sf-card');
  const keys = Object.keys(q.options);
  const optHtml = keys.map((k,i)=>`
...
// Guard: if somehow renderQuestion is reached for an already-answered question, redirect to review
...
function openTableModal(){
  const q = state.qIdx < QUESTIONS.length ? QUESTIONS[state.qIdx] : null;
  const backdrop = document.getElementById('sf-modal-backdrop');
  const body = document.getElementById('modal-body');
  const qnum = document.getElementById('modal-qnum');

  if(q && q.tables){
    qnum.textContent = `Q${state.qIdx+1} / ${QUESTIONS.length}`;
    body.innerHTML = parseTableText(q.tables);
  } else {
    qnum.textContent = q ? `Q${state.qIdx+1}` : '';
    body.innerHTML = `<div class="sf-modal-empty">No reference table for this question.</div>`;
  }
  backdrop.classList.add('open');
  document.getElementById('btn-modal-close').focus();
  // render math inside modal
  if(window.MathJax && MathJax.typesetPromise){
...
document.getElementById('btn-modal-close').addEventListener('click', closeTableModal);
document.getElementById('sf-modal-backdrop').addEventListener('click', e=>{
  if(e.target === document.getElementById('sf-modal-backdrop')) closeTableModal();
});
document.getElementById('btn-view-table').addEventListener('click', openTableModal);

function updateViewTableBtn(){
  const btn = document.getElementById('btn-view-table');
  // Show on all phases except final
...
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape'){
    const backdrop = document.getElementById('sf-modal-backdrop');
    if(backdrop.classList.contains('open')){ closeTableModal(); return; }
  }

  // T opens table modal from any phase (except if modal already open)
  if((e.key==='t'||e.key==='T') && !e.ctrlKey && !e.metaKey && !e.altKey){
    const backdrop = document.getElementById('sf-modal-backdrop');
    if(!backdrop.classList.contains('open') && state.phase!=='final'){
      const tag = document.activeElement?.tagName;
      if(tag!=='TEXTAREA'&&tag!=='INPUT'){ e.preventDefault(); openTableModal(); return; }
    }
  }

  // Don't intercept further if modal is open
